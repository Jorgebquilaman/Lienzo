...
namespace Lienzo.Application.Commands.UpdateActividad;

public record UpdateActividadCommand(Guid Id, CreateActividadRequest Request) : IRequest<Result<ActividadDto>>;
...
public class UpdateActividadCommandHandler : IRequestHandler<UpdateActividadCommand, Result<ActividadDto>>
{
    ...
    public UpdateActividadCommandHandler(IUnitOfWork unitOfWork, IAuthService authService, ICurrentUserService currentUser)
    {
    ...
    public async Task<Result<ActividadDto>> Handle(UpdateActividadCommand command, CancellationToken ct)
    {
        var actividad = await _unitOfWork.Actividades.GetWithDetailsAsync(command.Id);
        if (actividad is null)
            return Result<ActividadDto>.Failure("Actividad no encontrada", "NOT_FOUND");

        actividad.UpdateInfo(command.Request.Nombre, command.Request.CodigoMateria, command.Request.PeriodoId, command.Request.CarreraId);

        // Remove existing ActividadDocente records and re-add
        await _unitOfWork.Actividades.RemoveAllDocentesAsync(command.Id);

        if (command.Request.DocenteIds is not null)
        {
            foreach (var docenteId in command.Request.DocenteIds.Distinct())
                await _unitOfWork.ActividadDocentes.AddAsync(new ActividadDocente(command.Id, docenteId));
        }
        ...
        {
            actividad.RemoveSchedule();
        }

        await _unitOfWork.SaveChangesAsync(ct);

        // Generate or remove reservations from schedule
        var saved = await _unitOfWork.Actividades.GetWithDetailsAsync(command.Id);
        ...
        {
            await _unitOfWork.Reservations.DeleteByActividadIdAsync(command.Id);
        }
        await _unitOfWork.SaveChangesAsync(ct);

        saved = await _unitOfWork.Actividades.GetWithDetailsAsync(command.Id);
    ...
    public async Task GenerateReservationsFromScheduleAsync(Actividad actividad)
    {
        var periodo = await _unitOfWork.Periodos.GetByIdAsync(actividad.PeriodoId);
        if (periodo is null) return;

        // Remove old reservations for this actividad
        await _unitOfWork.Reservations.DeleteByActividadIdAsync(actividad.Id);
